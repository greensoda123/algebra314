...
A spacing of 1.45m caused the chain to fail, yet increasing it slightly to 1.50m allowed the entire sequence to continue.
...
In the UK, the standard dimensions for a brick are height $h = 2.15$ m and width $w = 0.65$ m:

$d_{\max} = \sqrt{2.15^2 + 0.65^2} - 0.65$

$= \sqrt{5.045} - 0.65$

$\approx 2.246 - 0.65 = 1.596 \text{ m}$

So the bricks cannot be spaced more than approximately $1.596$ m apart.
...
At a spacing of $1.50$ m, the chain successfully continues:
...
What I find interesting is that the distances between the bricks had to be very precise for the double domino effect to take place. For example, 1.5m distance works pretty well, but 1.47m was too small for there to be enough space for the bricks to fall in a straight line
